...
Windows는 PowerShell 또는 git-bash 둘 다 가능. 본 문서 명령은 git-bash 기준.
...
| 5     | 시드 데이터 / 테스트              | priority_rules / fact_definitions / sample products + e2e   |
...
```bash
# 루트에서
pnpm dlx @nestjs/cli@latest new backend --package-manager pnpm --skip-git
```

> NestJS CLI가 디렉터리를 생성한다. `backend/AGENTS.md`, `backend/CLAUDE.md`, `backend/README.md`가 이미 존재하므로 충돌 발생 시 NestJS 생성 파일을 우선시키지 말고 우리 파일을 보존한다 (`AGENTS.md` 등은 NestJS 템플릿에 없음 → 충돌 없음).
...
# 개발 의존성
pnpm add -D @types/cookie-parser @types/uuid
...
```bash
mkdir -p src/{lib,types,config,repositories,services,providers,controllers,modules}
```
...
- `src/main.ts`: cookie-parser, helmet, cors, validationPipe, swagger
...
pnpm run start:dev
# 다른 터미널
curl http://localhost:4000/health   # 200 응답 확인
pnpm prisma studio                  # DB 시각 확인
```
...
(`--no-eslint`는 root 통합 설정 후 일괄 적용 위해 비활성. 필요하면 켜도 됨.)
...
```bash
pnpm dlx shadcn@latest add button card input label badge dialog select tabs tag tooltip
```
...
```bash
mkdir -p src/{lib,types,config,api,hooks,store,components/ui}
mkdir -p src/components/{landing,priority-gate,category,product,traceback}
mkdir -p src/app/{priority-gate,category-decision,product-matrix,products,reaction-traceback}
mkdir -p src/app/category-decision/\[category\]
...
각 화면 작업 전 다음 4개를 모두 확인:

- [ ] `docs/page_content_specification_revised.md`의 해당 섹션 (카피/CTA/슬롯)
...
| context_questions + visibility       | `docs/page_content_specification_revised.md` + `matching_rules_revised.md` 4장 |
...
| brands + sample products (각 5개)    | 임시 더미 (실제 큐레이션은 Phase 6 이후)                    |
| ingredients + ingredient_groups      | 추후 (Phase 6 — Traceback 활성화 시)                        |
...
RUN pnpm --filter backend run prisma:generate && pnpm --filter backend run build
...
`infra/docker/Dockerfile.frontend`도 동일 패턴 (Next.js standalone output 사용).
...
DATABASE_URL="..." pnpm --filter backend prisma migrate deploy
```
...
on:
  workflow_dispatch:
    inputs: { env: { description: dev|prod, required: true } }
jobs:
  deploy:
    runs-on: ubuntu-latest
    permissions: { id-token: write, contents: read }
...
pnpm --filter backend prisma migrate dev --name <이름>
...
pnpm --filter frontend dlx shadcn@latest add <component>
pnpm --filter frontend run build
...
- Phase 5: `pnpm prisma db seed` 멱등 + `pnpm -r run test` 통과.
