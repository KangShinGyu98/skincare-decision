...
| Admin Rule Check                     | Rule 목록 조회                                    | GET        | `/admin/rules`                                  | Rule 목록을 DB `sort_order ASC` 순서로 조회하고 API에는 `sort_order`로 반환                                                     |
| Admin Rule Check                     | Rule 상세 조회                                    | GET        | `/admin/rules/:ruleId`                          | Rule 기본값, 연결 question/variant, condition 원본값, CTA 정보 조회                                                             |
| Admin Rule Check                     | Rule 표시 순서 저장                               | PATCH      | `/admin/rules/sort_order`                       | 정렬된 `ruleIds` 전체 목록을 검증한 뒤 DB `sort_order` 값을 1부터 재생성                                                        |
| Admin Rule Check                     | Rule 활성 상태 변경                               | PATCH      | `/admin/rules/:ruleId/status`                   | `priority_rules.is_active` 변경                                                                                                 |
| Admin Rule Check                     | Rule 관리자 메모 저장                             | PATCH      | `/admin/rules/:ruleId/admin-note`               | `priority_rules.admin_note` 저장                                                                                                |
...
- `GET /product-categories`는 `id`, `key`, `name`, `description`, `sortOrder`를 반환하고 `sortOrder ASC, key ASC` 순서로 정렬한다.
