...
> 목적: `product_attribute_schema_revised.md`와 `matching_rules_revised.md` 기준으로 실제 화면에 노출되는 텍스트, 질문, 데이터 슬롯, CTA, 동적 결과를
> 정의한다.  
...
피부가 건조한 거 뿐인데, 스킨/로션/앰플/세럼 뭐 사야할지 모르겠나요?  
로션 하나 고르는데도, 성분·제형·끈적임·휴대성까지 따질게 너무 많지 않나요?
...
| sort_order | category_key  | 라벨        | 이동                                      |
| ---------- | ------------- | ----------- | ----------------------------------------- |
| 10         | `toner`       | 토너        | `/category-decision?category=toner`       |
| 20         | `sunscreen`   | 선크림      | `/category-decision?category=sunscreen`   |
| 30         | `serum`       | 세럼        | `/category-decision?category=serum`       |
| 40         | `lipcare`     | 립케어      | `/category-decision?category=lipcare`     |
| 50         | `moisturizer` | 로션 / 크림 | `/category-decision?category=moisturizer` |
| 60         | `cleanser`    | 클렌저      | `/category-decision?category=cleanser`    |
...
예: `acne`, `recurring_trouble`, `breakout_reaction`, `sensitivity_reaction`은 `life.recent_irritation`,
`routine.cleansing_stable`, `routine.eye_irritation_history`, `routine.cleansing_before_sleep`,
`routine.pillowcase_change_cycle`, `routine.morning_face_condition`을 먼저 확인하고, `dryness`, `tightness`, `flaky_texture`는
`routine.recent_dry_tight`, `life.outdoor_activity`, `routine.sunscreen_frequency`, `routine.bedtime_routine`을 먼저 확인한다.
...
아래 key는 `product_matrix_filter_definitions.key`이며, 실제 attribute 기반 조건은 `product_filter_definitions`를 거쳐 해석한다. 복합/시스템 필터는
`product_matrix_filter_definitions.definition_kind = COMPUTED`로 두고 service handler가 처리한다.
...
상세 seed row는 `matching_rules_revised.md`의 `product_filter_definitions` / `product_matrix_filter_definitions` /
`question_filter_mappings` seed를 사용한다.
